...
import {
  executeRepoSeoPlan,
  type RemoteClient,
} from "../scripts/lib/repo-seo-apply.js";
import {
  replaceManagedBlock,
  validateRepoSeoPlan,
  type RepoSeoPlan,
...
async function fixtureRoot(repositories: PublicRepository[]): Promise<string> {
  ...
  const schema = await readFile(
    resolve(process.cwd(), "schemas/repo-seo-actions.schema.json"),
    "utf8",
  );
  const dataset: GithubDataset = {
    ...
    owner: "HSBL-ko-gyo",
    visibility: "public",
    filters: { forksExcluded: true, allowedForks: [] },
...
describe("README safety", () => {
  ...
  it("rejects a README action whose cached bytes do not match baseSha", async () => {
    ...
    await expect(validateRepoSeoPlan(root, plan)).rejects.toThrow(
      "Cached README bytes do not match base SHA",
    );
  });
});
...
describe("action plan safety", () => {
  it("rejects action counts above the policy limit", async () => {
    ...
    const plan: RepoSeoPlan = {
      ...
      actions: repos.map((repo) => ({
        ...
        repo: repo.name,
        description: "公開ツールの安全な説明",
      })),
    };
    await expect(validateRepoSeoPlan(root, plan)).rejects.toThrow(
      "Repository metadata action limit exceeded",
    );
  });
  ...
  it("rejects forbidden repository operations at schema validation", async () => {
    ...
    await expect(validateRepoSeoPlan(root, forbidden)).rejects.toThrow(
      "SEO action schema validation failed",
    );
  });
  ...
  it("does not call any remote function during dry-run", async () => {
    const plan: RepoSeoPlan = {
      ...
      sourceCollectedAt: "2026-01-01T00:00:00Z",
      actions: [
        { type: "repository-metadata", repo: "demo", description: "説明" },
      ],
    };
    const dataset: GithubDataset = {
      schemaVersion: 1,
    ...
    const context: ValidatedPlanContext = {
      plan,
      policy: policy(),
      dataset,
      repositories: new Map([["demo", dataset.repositories[0]]]),
    };
    let calls = 0;
