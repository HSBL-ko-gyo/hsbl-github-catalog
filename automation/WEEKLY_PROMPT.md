...
3. 既存の公開作品について、次の事実を同期する。
   ...
   - pushedAt / updatedAt
   - archived状態
4. 既存の `summary`, `description`, 本文は、事実誤認、リンク切れ、重大な用途不明がある場合だけ大きく修正する。既に本人文体で書かれた文章を、無難な製品説明へ戻さない。固定6節を復活させず、全作品を同じ見出し順へ揃えない。既存ページ本文の大きな修正は最大3件まで。
...
6. 次をすべて満たす明確な作品だけ、最大8件の公開ページを `draft: false` で作る。
   ...
   - 会社、顧客、受託、個人情報、内部資料の懸念がない
     新規作品も既存作品の本人文体へ合わせ、2〜4個程度の内容固有のH2で書く。企業やサービスの紹介文にせず、機能や動機を創作しない。`だるかったので作った`などの口癖は根拠がある時だけ使い、機械的に散らさない。
