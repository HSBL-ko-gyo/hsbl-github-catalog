...
const httpsUrl = z.url().refine((value) => value.startsWith("https://"), "HTTPS URLのみ指定できます");
...
export function isPublishedProject(project: { data: Pick<ProjectFrontmatter, "draft"> }): boolean {
  return !project.data.draft;
}

export function sortProjects<T extends { data: Pick<ProjectFrontmatter, "featured" | "title"> }>(projects: T[]): T[] {
  return [...projects].sort(
    (a, b) => Number(b.data.featured) - Number(a.data.featured) || a.data.title.localeCompare(b.data.title, "ja"),
  );
}
